angular.module("superApp", [ "ngAnimate", "ngDraggable", "ngDialog" ]).directive("heroBackground", [ "$rootScope", "$timeout", function($rootScope, $timeout) {
    return {
        ...
        link: function(scope, element, attr) {
            ...
            if (heroesColors.hasOwnProperty(scope.heroName.toLowerCase().replace(" ", "_"))) {
                ...
                background += ")", element.css("background-image", background);
            }
        }
    };
    ...
    $scope.loading = !0, $scope.heroes = [], init(), heroService.key().then(function() {
        heroService.get().then(function(data) {
            console.log("hero data"), console.log(data), $scope.heroes = data, $scope.loading = !1;
        });
    }), $scope.create = function(newHero) {
        console.log("creating....."), console.log(newHero), $scope.isMerge && (newHero.powers = $scope.merge.powers), 
        $scope.attributes.forEach(function(attribute) {
            newHero.attributes[attribute] = parseInt(newHero.attributes[attribute]);
        }), console.log(newHero), heroService.post(newHero).then(function(response) {}), 
        $scope.heroes.unshift(newHero), init();
        ...
        console.log(this), this.power.length <= 0 && $scope.newHero.powers.splice(this.$index, 1);
        ...
        $scope.merge[key] = hero, console.log($scope.merge.hero1), $scope.startMerge() && ($scope.isMerge = !0, 
